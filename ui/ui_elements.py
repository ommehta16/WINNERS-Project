...
class Button:
    def __init__(self,_location:tuple[float,float],_size:tuple[float,float],_on_click,_color="gray",_hover_color="dark gray",_outline_color="black",_text="",_font="Calibri",_fontsize=12,outline_width=1):
        ...
        self.image: pygame.Surface = None # self.image and self.rendered_text are assigned by add_image and generated by update respectively, so we initialize to None
    ...
    def draw(self):
        ...
        pygame.draw.rect(screen,self.colors["outline"],self.rect,self.out_size) # Draw the outline as a rect on top of the background
        
        if self.image != None: # image + text --> stacked, image w/o text --> center, only text --> center
    ...
    def update(self, click:bool):
        '''
        Run `self.on_click` if applicable, rescale/regenerate the image and text.
        '''
        did_action = False # Tracks whether we ran self._onclick
        ...
        if self.text: #re-render & the text to fit inside the button
        ...
        if self.image: # re-size the image to fit with the text in the button: don't show the image at all if the scaled size would be miniscule or negative
            text_height = 0
    ...
    def update_font(self,size:int,font_name:str):
        '''
        Change the size and type of the font being used. Re-renders `self.rendered_text` using the specified new font
        '''
        self.font = pygame.font.SysFont(font_name,size)
        self.rendered_text = self.font.render(self.text,True,self.colors["outline"])
    def set_image(self, image_path:str):
        '''
        Give the button an image, and determine how to render it
        '''
        ...
        max_w = self.rect.width # max bounds are hard limits: based on the size of the button's borders and on the size of the text included
...
class ButtonGrid:
    ...
    def __init__(self,loc:tuple[float],dims:tuple[float],grid_sz:tuple[int], bg_col="light gray",col="gray",hov_col="dark gray",outline_col="black",outline_width=1,font_size=12,buttons:list=[]):
        ...
        for action, image_path, text in buttons: # Loop through the button initializer list and call `self.add_button` on each entry
            self.add_button(_onclick=action,_text=text,_font_size=font_size)
            if len(image_path): self.buttons[-1].set_image(image_path) #add an image to the button if one was requested

    def add_button(self, _onclick=lambda: 0, _text: str = "", _font: str = "Calibri", _font_size: int = 12):
        '''
        Add a single button to the grid
        '''
        pos = self.rect.topleft
        prev_rect = None
        if len(self.buttons): prev_rect = self.buttons[-1].rect
        if self.grid_size[0] == 0: # Unlimited x-axis, so we don't care about moving the next button down a line
            ...
            if prev_rect != None and len(self.buttons) % self.grid_size[0] == 0: # Go to the next line if the previous row was filled
    ...
    def draw(self):
        '''
        Draw all of the grid's buttons onto the active display
        '''
        screen = pygame.display.get_surface()
        
        pygame.draw.rect(screen,self.colors["background"],self.rect) # Draw the background to the FULL assigned size of the grid (*not* the filled size)
        
        for button in self.buttons:
            button.draw()
    def update(self,click:bool) -> bool:
        '''
        Update all the buttons in self, and resize them if necessary. Returns True if one or more buttons was clicked and ran `self.on_click`
        '''
        ran = False
...
class Prompt:
    def get_file_open(file_type:str = "All files (*.*)") -> str:
        ...
        Takes a file type in the format "[NAME] (*.[ext1] *.[ext2] etc.)"\n
        Returns a string corresponding to the path of the selected file'''
    ...
    def get_file_save(file_type:str = "All files (*.*)") -> str:
        ...
        Takes a file type in the format "[NAME] (*.[ext1] *.[ext2] etc.)"\n
        Returns a string corresponding to the path of the file to save to'''
        app = QApplication(sys.argv)
...
class Slider:
    '''
    A slider that can be dragged for analog-like input
    '''
    ...
    def update(self,clicked:bool,moved:bool) -> None:
        '''
        Update the slider's `self.value` using the mouse's current position, and using the clicked/moved status of the mouse.
        '''
        if clicked:
    ...
    def draw(self) -> None:
        '''
        Draws the slider. `self.rect` is used as the main body of the slider
        '''
        screen = pygame.display.get_surface()
    ...
    def get_value(self) -> int:
        '''Returns the value of the slider as a number between max and min'''
        return self.progress*(self.max-self.min) + self.min

def title_screen(chng_img,BACKGROUND_COLOR) -> None:
    '''Run the short title sequence, prompting the user to enter an image before starting'''
    on_title = True
    # Set the text and fonts that we'll use in the title screen
    ...
    screen = pygame.display.set_mode((800,600))  # Makes window NON-resizable, and fixes the size to be (fairly) small -- there's not much to see on this screen!
    def title_img_set():
        nonlocal on_title
        if chng_img():
            on_title = False
    ...
    while on_title:
        ...
        clicked = False
        for event in pygame.event.get(): # very simple event handler: only does clicks and quitting out of the app
            if event.type == pygame.QUIT: quit()
            if event.type == pygame.MOUSEBUTTONDOWN: clicked = True
        start_up_button.update(clicked) # update the file input button based on whether the user has clicked or not
...
class adjustmentTile:
    '''
    A pane on the screen that shows the adjustment options for any given effect
    '''
    def __init__(self,top:int,preview,apply):
        screen = pygame.display.get_surface()
        self.hidden = True
        self.unit = "%"

        self.adjust = ButtonGrid([0, top], [screen.get_size()[0] * 1/4, 20], [3, 1])
        self.adjust.add_button(_text="CANCEL",_onclick=lambda:0)
    ...
    def update(self,clicked:bool,moved):
        ...
        if self.unit == "°": slider_val = int(3.6*self.slider.get_value())
        else: slider_val = int(self.slider.get_value())
        self.slider_value_text = self.slider_font.render(f'{slider_val}{self.unit}',1,self.slider_color)
